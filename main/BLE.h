...
#define SAMPLE_DEVICE_NAME          "LOP2_NHOM2"
...
static const uint8_t char1_name[]  = "Data from BLE";
static uint8_t value_MQTT[SHORT_CHAR_VAL_LEN] = {};
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_1_SHORT_WR, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE | ESP_GATT_PERM_READ_ENC_MITM,
      SHORT_CHAR_VAL_LEN, sizeof(value_MQTT), (uint8_t *)value_MQTT}},
...
char* write_value_to_string(const uint8_t* value, size_t length) {
    // Tạo một mảng ký tự đủ lớn để chứa giá trị chuyển đổi
    char* str = (char*)malloc((length*2 + 1) * sizeof(char));
    if (str == NULL) {
        // Xử lý lỗi khi không thể cấp phát bộ nhớ
        printf("Failed to allocate memory for string conversion\n");
        return NULL;
    }

    // Chuyển đổi từng byte thành ký tự tương ứng và gán vào chuỗi kết quả
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if (gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle) {
                    ...
                    ESP_LOGI(EXAMPLE_TAG, "param->write.value:");
                    BLE_data = write_value_to_string(param->write.value, param->write.len);
                    if (BLE_data != NULL) {
                        ESP_LOGI(EXAMPLE_TAG, "%s", BLE_data);
                    }
                    // So sánh giá trị của event->data với chuỗi "oke"
                    if (strncmp(BLE_data, "oke", 3) == 0) {
                        flag_BLE = true;
                    } 
                    else{
                        value_BLE = strdup(BLE_data);
                    }
                }
                /* send response when param->write.need_rsp is true*/
                if (param->write.need_rsp){
...
void disable_ble() {
    ...
    if (ret != ESP_OK) {
        ESP_LOGE("disable_ble", "Failed to disable bluedroid: %s", esp_err_to_name(ret));
        return;
    }

    ret = esp_bluedroid_deinit();
    if (ret != ESP_OK) {
        ESP_LOGE("disable_ble", "Failed to deinitialize bluedroid: %s", esp_err_to_name(ret));
        return;
    }

    ret = esp_bt_controller_disable();
    if (ret != ESP_OK) {
        ESP_LOGE("disable_ble", "Failed to disable controller: %s", esp_err_to_name(ret));
        return;
    }

    ret = esp_bt_controller_deinit();
    if (ret != ESP_OK) {
        ESP_LOGE("disable_ble", "Failed to deinitialize controller: %s", esp_err_to_name(ret));
        return;
    }

    ESP_LOGI("disable_ble", "BLE disabled successfully");
}
void enable_ble() {
    esp_err_t ret;   
   ...
   ret = esp_bt_controller_init(&bt_cfg);
    ...
    ESP_LOGI("enable_ble", "BLE enabled successfully");
}
void BLE_security(){
    ...
    /* If your BLE device acts as a Slave, the init_key means the types of keys you hope the master distributes to you,
    and the response key means the key you can distribute to the Master;
    If your BLE device acts as a Master, the response key means the types of keys you hope the slave distributes to you,
    and the init key means the key you can distribute to the slave. */
